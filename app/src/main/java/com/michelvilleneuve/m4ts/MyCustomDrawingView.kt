...
// Ensure spacing is positive
 ...
 //       TOP, RIGHT, BOTTOM, LEFT }
    ...
    interface Shape : Parcelable, Serializable {
     ...
     data class Line(
         ...
         val start: PointF, val end: PointF, var locked: Boolean = false, var color: Int = Color.BLACK
    ) : Shape{
    ...
    data class Text(
        ...
        //Parcelable, Serializable {
        override fun contains(x: Float, y: Float): Boolean {
            // Implement your logic for checking if a point is within the text's bounds
            return Math.abs(this.x - x) < 20 && Math.abs(this.y - y) < 20 // Example buffer
        }
        val width: Float
        val height: Float
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
      ...
      //                      DrawingMode.ERASE -> eraseElement(startX, startY)
                ...
                MotionEvent.ACTION_UP -> {
                    ...
                    // Handle handle selection release
                    if (selectedHandle != null) {
    ...
    fun handleLongPress(e: MotionEvent) {
        ...
        for (line in lines) {
            if (isPointNearHandle(x, y, line.startX, line.startY)) {
                ...
                Log.d("TAG", "Long Press the start or end")
                selectedLine = line
                selectedHandle = Handle.MIDDLE
                handleSelected = true
                break
 ...
 //       val threshold = 20f // Adjust the threshold value as needed
    ...
    private fun handleActionDown(event: MotionEvent, x: Float, y: Float) {
        ...
        when (currentDrawingMode) {
            ...
            DrawingMode.RECTANGLE -> {
                rectangles.add(Rectangle(startX, startY, startX, startY, startX, startY, startX, startY))
            }
            DrawingMode.FREEHAND -> {
            ...
            DrawingMode.ERASER_EFFECT -> {
                eraserEffectPath.moveTo(snappedX, snappedY)
            }
            DrawingMode.TEXT -> {
                // Handle text drawing
    ...
    private fun handleActionMove(event: MotionEvent, x: Float, y: Float) {
        ...
        Log.d("TAG", "before snapToGrid - $startX")

        val (snappedX, snappedY) = snapToGrid(x, y)

        Log.d("TAG", "after snapToGrid startX and snappedX- $startX - $snappedX")
            ...
            if (isDrawing) {
                ...
                when (currentDrawingMode) {
                    ...
                    DrawingMode.ERASER_EFFECT -> {
                        ...
                        eraseElementsAlongPath(eraserEffectPath)
                        eraserTrail.add(PointF(snappedX, snappedY))  // Add point to the trail
                        invalidate()
                    }
        ...
        if (selectedHandle != null) {
            selectedLine?.let {
                when (selectedHandle) {
                    Handle.START -> {
                        Log.d("TAG", "action move before snap it startX and snappedX - ${it.startX} - $snappedX")
                        val (newX, newY) = snapToHorizontalOrVertical(snappedX, snappedY, it.endX, it.endY)
                        it.startX = newX
                        it.startY = newY
                        Log.d("TAG", "action move after snap it startX and snappedX - ${it.startX} - $snappedX")
                    }
                    Handle.END -> {
                    ...
                    Handle.MIDDLE -> {
                        ...
                        Log.d("TAG", "action move middle it startX and snappedX - ${it.startX} - $snappedX")
                        it.startX += dx
                        it.startY += dy
                        it.endX += dx
                        it.endY += dy
    ...
    //    val gridSize = 10f
        ...
        lines.forEach { line ->
            ...
            Log.d("TAG", "onDraw near point snappedStartX: $snappedStartX")
            val (snappedEndX, snappedEndY) = getNearestSnapPoint(line.endX, line.endY)
            Log.d("TAG", "onDraw near point 2 snappedEndX: $snappedEndX")

            // Snap the line to horizontal or vertical orientations if close to those angles
            val (finalStartX, finalStartY) = snapToHorizontalOrVertical(snappedStartX, snappedStartY, snappedEndX, snappedEndY)
            Log.d("TAG", "onDraw snap1 finalStartX: $finalStartX - line ${line.startX} - snappedStartX $snappedStartX")
            val (finalEndX, finalEndY) = snapToHorizontalOrVertical(snappedEndX, snappedEndY, snappedStartX, snappedStartY)
            Log.d("TAG", "onDraw snap2 finalEndX: $finalEndX - line ${line.endX} - snappedEndX $snappedEndX")



            Log.d("TAG", "return from snapToHorizontalOrVertical start : line ${line.startX} - $startY")
            Log.d("TAG", "return from snapToHorizontalOrVertical end : line ${line.endX} ")
            // Draw the line with the snapped coordinates

            canvas.drawLine(finalStartX, finalStartY, finalEndX, finalEndY, paint)

        //    canvas.drawLine(snappedStartX, snappedStartY, snappedEndX, snappedEndY, paint)
            Log.d("TAG", "onDraw after draw line finalStartX: $finalStartX")
            Log.d("TAG", "onDraw after draw line finalEndX: $finalEndX")
        ...
        // Draw texts
        texts.forEach {
            val (snappedX, snappedY) = getNearestSnapPoint(it.x, it.y)
            canvas.drawText(it.text, snappedX, snappedY, textPaint)
            if (it == selectedShape) {
                drawHandle(canvas, snappedX, snappedY)
            }
        }
 ...
 //           return Pair(endX, endY) // Return original points if snapping is disabled
 //       }
        ...
        return when {
            ...
            angle in -5f..5f || angle in 175f..185f -> Pair(endX, startY)

            // Snap to vertical down (90 degrees)
            angle in 85f..95f -> Pair(startX, endY)

            // Snap to vertical up (270 degrees)
            angle in -95f..-85f || angle in 265f..275f -> Pair(startX, endY)
